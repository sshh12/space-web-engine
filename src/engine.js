...
import { SYSTEM, AU, assertPaletteRecipe, assertFigureRecipe, assertGiantRecipe, assertRingRecipe, assertGiantSystem, assertRingSystem } from './core/recipe.js';
// Phase B: the coma/tail emission look joins the point-tier flux hand-down
// (the system view renders the tail; core/smallbody.js owns the pure law).
import { comaApparentFlux } from './core/smallbody.js';
...
import { ephemeris, bodyToInertial, mulMM, transpose, orbitalPeriodS, spinPeriodS } from './core/frames.js';
...
import { hostRelativePosition, starterSystemPose } from './core/navigation.js';
// Phase W (round 24): the [time-field] warp policy — representation selection
// is a pure function of the DECLARED warp; hysteresis lives only on the live
// slider path; above-band forms are Jensen-honest (core/warp.js).
import {
  makeWarpBandMachine, warpRepresentation, oceanTimeS,
  AURORA_MEAN_PULSE, STARTER_SYSTEM_WARP,
} from './core/warp.js';
...
import { makeStarPoints, makeBodyPoints } from './render/stars.js';
...
import { cloudKeyOf, driftPhase, alphaMeanLit, assertCloudRecipe, cloudCovJS, cloudShadeJS, meanBlockOf, CLOUD_W, CLOUD_H, MAX_DECKS } from './core/cloudcore.js';
import { epochFromViews, solvePhaseTday, viewsAtEpoch } from './core/time.js';
import { systemIdentity } from './core/system.js';
// Phase E (round 25): the editor's pure laws — taxonomy diff classification,
// preflight-on-a-copy, and the λ-continuity solve — live in core/editor.js;
// the engine only DISPATCHES the classified invalidation plan.
import {
  classifyEdit, preflightEditedSystem, withLambdaContinuity,
  hasSeasonalDecks, insolationMinLevel,
} from './core/editor.js';
import { invalidationLevel } from './core/bakecore.js';
import { resolveSpec } from './scenespec.js';
import { assertMechanicsSystem } from './core/mechanics.js';
import {
  MAX_BODY_SLOTS, MAX_ECLIPSE_OCCLUDERS, assertSkyUniformBudget,
  makeBodyLayerMap, layerForBody, assertStructuredCloneSafe,
  assignBodyRepresentations, discIntegratedFlux, makeGenerationFence, makeJitQueue,
} from './core/capacity.js';

export function createEngine(canvas, options = {}) {
const host = options.host ?? window;
const doc = options.document ?? document;
const q = new URL(options.url ?? location.href).searchParams;
let loadedSystem = options.system ? structuredClone(options.system) : SYSTEM;
let bodyLayers = makeBodyLayerMap(loadedSystem);
const generationFence = makeGenerationFence();
// Phase E: canonical systems resolvable by id (spec.system strings) and the
// known-recipe hashes that distinguish a shipped system from an EDITED one —
// an edited system travels as an inline payload in every spec it appears in.
const systemsById = new Map([[SYSTEM.id, SYSTEM]]);
if (options.system) systemsById.set(options.system.id, options.system);
for (const s of Object.values(options.systems ?? {})) systemsById.set(s.id, s);
const knownSystemHashes = new Set([...systemsById.values()].map((s) => systemIdentity(s).recipeHash));
const loadedBodyById = (id) => loadedSystem.bodies.find((b) => b.id === id);
const initialBodyId = options.initialBodyId
  ?? (options.system ? loadedSystem.bodies[0].id : (loadedBodyById('tellus')?.id ?? loadedSystem.bodies[0].id));
const FAST = options.fast ?? q.get('fast') === '1';
const $ = (id) => doc.getElementById(id);
const viewportSize = () => ({
  width: options.width ?? canvas.clientWidth ?? host.innerWidth,
  height: options.height ?? canvas.clientHeight ?? host.innerHeight,
});

const renderer = new THREE.WebGLRenderer({
  canvas, antialias: !FAST, logarithmicDepthBuffer: true, powerPreference: 'high-performance',
});
const skyUniformBudget = assertSkyUniformBudget(renderer.getContext().getParameter(renderer.getContext().MAX_FRAGMENT_UNIFORM_VECTORS));
renderer.setPixelRatio(FAST ? 1 : Math.min(devicePixelRatio, 2));
{ const { width, height } = viewportSize(); renderer.setSize(width, height); }
...
const camera = new THREE.PerspectiveCamera(55, initialSize.width / initialSize.height, 0.3, 6e8);
...
const shared = {
  ...
  uNumOcc: { value: 0 },
  uOccPos: { value: Array.from({ length: MAX_ECLIPSE_OCCLUDERS }, () => new THREE.Vector3()) },
  uOccR: { value: Array(MAX_ECLIPSE_OCCLUDERS).fill(0) },
  uOccAnn: { value: Array.from({ length: MAX_ECLIPSE_OCCLUDERS }, () => new THREE.Vector3()) },
...
assertGiantSystem(loadedSystem); assertRingSystem(loadedSystem);
...
let NB = loadedSystem.bodies.length;
let discAtlas = new Uint8Array(DISC_W * DISC_H * 4 * NB);
let discTex = new THREE.DataArrayTexture(discAtlas, DISC_W, DISC_H, NB);
...
let CLOUD_LAYERS = NB * MAX_DECKS;
let cloudData = new Uint8Array(CLOUD_W * CLOUD_H * 4 * CLOUD_LAYERS);
let cloudTex = new THREE.DataArrayTexture(cloudData, CLOUD_W, CLOUD_H, CLOUD_LAYERS);
...
function allocateAtlases(system) {
  const layers = system.bodies.length;
  const nextDiscData = new Uint8Array(DISC_W * DISC_H * 4 * layers);
  const nextDiscTex = new THREE.DataArrayTexture(nextDiscData, DISC_W, DISC_H, layers);
  nextDiscTex.format = THREE.RGBAFormat; nextDiscTex.type = THREE.UnsignedByteType;
  nextDiscTex.wrapS = THREE.RepeatWrapping; nextDiscTex.wrapT = THREE.ClampToEdgeWrapping;
  nextDiscTex.minFilter = THREE.LinearMipmapLinearFilter; nextDiscTex.magFilter = THREE.LinearFilter;
  nextDiscTex.generateMipmaps = true; nextDiscTex.needsUpdate = true;
  const cloudLayers = layers * MAX_DECKS;
  const nextCloudData = new Uint8Array(CLOUD_W * CLOUD_H * 4 * cloudLayers);
  const nextCloudTex = new THREE.DataArrayTexture(nextCloudData, CLOUD_W, CLOUD_H, cloudLayers);
  nextCloudTex.format = THREE.RGBAFormat; nextCloudTex.type = THREE.UnsignedByteType;
  nextCloudTex.wrapS = THREE.RepeatWrapping; nextCloudTex.wrapT = THREE.ClampToEdgeWrapping;
  nextCloudTex.minFilter = THREE.LinearMipmapLinearFilter; nextCloudTex.magFilter = THREE.LinearFilter;
  nextCloudTex.generateMipmaps = true; nextCloudTex.needsUpdate = true;
  return { layers, discData: nextDiscData, discTex: nextDiscTex, cloudLayers, cloudData: nextCloudData, cloudTex: nextCloudTex };
}
...
// the round-14 lesson). Rows carry their KIND — 'live' keyframe pair vs the
// Phase W above-band 'mean' raster — because both share the same atlas layers:
// an untagged row would let a mean raster keep rendering as a keyframe pair
// after a downward band crossing (the F2 stale-representation class).
const cloudState = { rows: new Map(), want: new Map(), meanInFlight: new Set(), pending: 0 };
const shineCovCache = new Map(); // bodyId -> { tq, alb } (per-world-minute)
shared.uCloudMap.value = cloudTex;

const vec3slots = () => Array.from({ length: MAX_BODY_SLOTS }, () => new THREE.Vector3());
const skyUniforms = {
  ...shared,
  ...
  uNumBodies: { value: 0 },
  uBodyDir: { value: vec3slots() },
  uBodyAngR: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyCol: { value: vec3slots() },
  uBodySun: { value: vec3slots() },
  uBodyAtlas: { value: discTex },
  uBodyR0: { value: vec3slots() },
  uBodyR1: { value: vec3slots() },
  uBodyR2: { value: vec3slots() },
  uBodyRowV: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyDiscLoaded: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyFlatAlb: { value: vec3slots() },
  ...
  uBodyFrostK: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyFrostP: { value: vec3slots() },
  uBodyFrostCol: { value: vec3slots() },
  ...
  uBodyHazeK: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyHazeCol: { value: vec3slots() },
  ...
  uBodyCloudN: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyCloudA: { value: Array.from({ length: MAX_BODY_SLOTS * MAX_DECKS }, () => new THREE.Vector4()) },
  uBodyCloudAlb: { value: vec3slots() },
  // round 18 — Phase 6 giant + ring. RUNTIME per-slot gates (default 0 ⇒ legacy
  // discs byte-identical). Giant/ring profiles are independent per resolved slot.
  ...
  uBodyGiant: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uBodyRing: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uGiantBand: { value: Array.from({ length: MAX_BODY_SLOTS * 8 }, () => new THREE.Vector4()) },
  uGiantBandN: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uGiantLimbExp: { value: Array(MAX_BODY_SLOTS).fill(1) }, uGiantLimbK: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uGiantStorm: { value: Array.from({ length: MAX_BODY_SLOTS }, () => new THREE.Vector4()) },
  uGiantStormCol: { value: vec3slots() },
  uGiantHex: { value: Array.from({ length: MAX_BODY_SLOTS }, () => new THREE.Vector4(0.9, 0, 0, 0)) },
  uGiantHexCol: { value: vec3slots() },
  uRingInner: { value: Array(MAX_BODY_SLOTS).fill(0) }, uRingOuter: { value: Array(MAX_BODY_SLOTS).fill(0) }, uRingRp: { value: Array(MAX_BODY_SLOTS).fill(0) },
  uRingGap: { value: Array.from({ length: MAX_BODY_SLOTS * 4 }, () => new THREE.Vector4()) },
  uRingCol: { value: vec3slots() },
  uRingTau: { value: Array(MAX_BODY_SLOTS).fill(0) }, uRingFsG: { value: Array(MAX_BODY_SLOTS).fill(0) },
...
const bodyPointTier = makeBodyPoints(shared, skyUniforms, withCommon, SCATTER_FOR_STARS, FAST ? 6 : 12, starDepth, 128);
starScene.add(bodyPointTier.points);

// ---- state ----
const worker = new Worker(new URL('./bake.worker.js', import.meta.url), { type: 'module' });
const loadedDiscs = new Set();
const assetPacks = new Map(), assetPending = new Set();
const coarseResidency = new Map();
const discJobs = makeJitQueue();
let discJobActive = false;
function dispatchDiscJob() {
  if (discJobActive) return;
  const bodyId = discJobs.shift();
  if (!bodyId) return;
  discJobActive = true;
  worker.postMessage(generationFence.stamp({ type: 'discmap', bodyId }));
}
function queueDiscJobs(currentId, start = true) {
  discJobs.clear(); discJobActive = false;
  for (const b of loadedSystem.bodies) discJobs.push(b.id, b.id === currentId ? -1 : 10);
  state.discPending = loadedSystem.bodies.length;
  if (start) dispatchDiscJob();
}
worker.addEventListener('message', (e) => {
  const m = e.data;
  if (!generationFence.accepts(m)) return;
  if (m.cache) state.workerCache = m.cache;
  if (!m.type && m.level === 0) {
    const rows = coarseResidency.get(m.bodyId) ?? new Map();
    rows.set(`${m.face}/0/0/0`, m); coarseResidency.set(m.bodyId, rows);
  }
  if (m.type === 'system') {
    if (state.interactiveMs != null) dispatchDiscJob();
    return;
  }
  if (m.type === 'assetpack') {
    assetPacks.set(m.bodyId, m.pack); assetPending.delete(m.bodyId);
    state.prewarmPending = assetPending.size;
    return;
  }
  if (m.type === 'clouds' || m.type === 'cloudmean') {
    cloudState.pending = Math.max(0, cloudState.pending - 1);
    if (m.type === 'cloudmean') cloudState.meanInFlight.delete(m.bodyId);
    const row = bodyLayers.get(m.bodyId);
    if (row >= 0 && row < NB && m.decks > 0) {
      cloudState.rows.set(m.bodyId, m.type === 'cloudmean'
        ? { kind: 'mean', k: -1, block: m.block, decks: m.decks, rgba: m.rgba }
        : { kind: 'live', k: m.k, decks: m.decks, rgba: m.rgba });
  ...
  if (row >= 0 && row < NB) {
    ...
    discTex.needsUpdate = true;
    loadedDiscs.add(m.bodyId);
  }
  state.discPending = Math.max(0, state.discPending - 1);
  discJobActive = false;
  dispatchDiscJob();
});
...
function requestClouds(b, k) {
  ...
  if (cur && cur.kind === 'live' && cur.k === k) return;
  if (cloudState.want.get(b.id) === k) return;
  cloudState.want.set(b.id, k);
  cloudState.pending++;
  worker.postMessage(generationFence.stamp({ type: 'clouds', bodyId: b.id, k }));
}
// Phase W above-band twin of requestClouds. At most ONE mean job per body is
// ever in flight (free-run at month/s would otherwise flood the worker with a
// new block every few frames — the register-#7 starvation, re-armed); the last
// delivered block keeps rendering while the next generates (the mean-row
// analog of the live path's k−1 rollover fallback), and a frozen capture's
// single block always resolves exactly before settle.
function requestCloudMean(b, block) {
  if (!b.clouds) return;
  const cur = cloudState.rows.get(b.id);
  if (cur && cur.kind === 'mean' && cur.block === block) return;
  if (cloudState.meanInFlight.has(b.id)) return;
  cloudState.meanInFlight.add(b.id);
  cloudState.want.delete(b.id); // a live-row want is obsolete above the band
  cloudState.pending++;
  worker.postMessage(generationFence.stamp({ type: 'cloudmean', bodyId: b.id, block }));
}
const state = {
  body: null, tiles: null, epochS: epochFromViews(loadedBodyById(initialBodyId), { season: 0.15, tday: 0.5 }, loadedSystem),
  tday: 0.5, season: 0.15, speed: options.viewClass === 'system' ? STARTER_SYSTEM_WARP : 60,
  // DECLARED warp (signed) — the [time-field] representation selector. `speed`
  // is the integration rate and equals warp in free-run; captures freeze speed
  // at 0 while warp keeps declaring which representation set renders (Phase W).
  warp: options.viewClass === 'system' ? STARTER_SYSTEM_WARP : 60,
  viewClass: options.viewClass === 'system' ? 'system' : 'surface',
  hostId: options.viewClass === 'system' ? 'star' : initialBodyId,
  travel: null, navMarkers: [], savedWarp: null,
  ...
  discPending: loadedSystem.bodies.length,
  systemEdited: false, // Phase E: true when loadedSystem's hash is not a shipped one
  workerCache: { tiles: 0, budget: 0, bakers: 0, evictions: 0 },
  prewarmPending: 0,
  loadStarted: performance.now(), interactiveMs: null,
...
// Phase W: the live band machine. Continuous slider sweeps take the
// hysteretic path; every capture/apply resets it to the pure selection.
const warpBands = makeWarpBandMachine();
warpBands.reset(state.warp);
...
preflightSystem(loadedSystem);
if (options.system) {
  generationFence.bump();
  worker.postMessage(generationFence.stamp({ type: 'system', system: loadedSystem }));
}
queueDiscJobs(initialBodyId, false);

const figOkCache = new Set(); // preflight once per body per session

function preflightSystem(system) {
  assertStructuredCloneSafe(system);
  assertMechanicsSystem(system);
  if (!system.bodies.length) throw new Error('capacity: a renderable system needs at least one body');
  const layers = makeBodyLayerMap(system);
  assertGiantSystem(system); assertRingSystem(system);
...
function rebuildBodyMenu(selectedId) {
  const bodySel = $('body');
  if (!bodySel) return;
  bodySel.innerHTML = '';
  for (const b of loadedSystem.bodies) {
    const opt = doc.createElement('option');
    opt.value = b.id; opt.textContent = b.name;
    bodySel.appendChild(opt);
  }
  bodySel.value = selectedId;
}

function setSystem(system) {
  // Clone and validate every datum before touching the live renderer. A failed
  // preflight leaves textures, worker generation, tiles and UI unchanged.
  const next = structuredClone(system);
  const nextLayers = preflightSystem(next);
  const atlases = allocateAtlases(next);
  const keepId = nextLayers.has(state.body?.id) ? state.body.id
    : nextLayers.has('tellus') ? 'tellus' : next.bodies[0].id;

  generationFence.bump();
  loadedSystem = next; bodyLayers = nextLayers;
  NB = atlases.layers; CLOUD_LAYERS = atlases.cloudLayers;
  const oldDiscTex = discTex, oldCloudTex = cloudTex;
  discAtlas = atlases.discData; discTex = atlases.discTex;
  cloudData = atlases.cloudData; cloudTex = atlases.cloudTex;
  skyUniforms.uBodyAtlas.value = discTex; shared.uCloudMap.value = cloudTex;
  loadedDiscs.clear(); cloudState.rows.clear(); cloudState.want.clear();
  cloudState.meanInFlight.clear(); cloudState.pending = 0;
  assetPacks.clear(); assetPending.clear(); state.prewarmPending = 0;
  coarseResidency.clear();
  shineCovCache.clear(); msCache.clear(); figOkCache.clear();
  state.discPending = next.bodies.length;
  state.loadStarted = performance.now(); state.interactiveMs = null;
  worker.postMessage(generationFence.stamp({ type: 'system', system: next }));
  queueDiscJobs(keepId, false);
  switchBody(keepId);
  state.epochS = epochFromViews(state.body, { season: state.season, tday: state.tday }, loadedSystem);
  rebuildBodyMenu(keepId);
  if (systemView) systemView.setSystem(loadedSystem);
  oldDiscTex.dispose(); oldCloudTex.dispose();
  const identity = systemIdentity(loadedSystem);
  state.systemEdited = !knownSystemHashes.has(identity.recipeHash);
  return identity;
}

// Phase E: after a generation bump, any in-flight cloud/assetpack reply is
// structurally dead (stale generation) — its pending count would never
// decrement and the settle predicate would stall. Re-derive the counters;
// the frame loop re-requests whatever is actually needed under the new
// generation (requests are idempotent and the worker caches are warm).
function resetAsyncPipelines() {
  cloudState.want.clear(); cloudState.meanInFlight.clear(); cloudState.pending = 0;
  assetPending.clear(); state.prewarmPending = 0;
  discJobs.clear(); discJobActive = false;
  for (const b of loadedSystem.bodies) if (!loadedDiscs.has(b.id)) discJobs.push(b.id, b.id === state.body?.id ? -1 : 10);
  state.discPending = loadedSystem.bodies.length - loadedDiscs.size;
}

/**
 * Phase E applyEdit — the ONLY live mutation path. Validate → preflight (on a
 * copy; a bad edit reports and changes nothing) → λ-continuity → classify the
 * diff through the taxonomy → invalidate exactly what the classes name, riding
 * one generation bump so stale worker replies are structurally dead. The
 * running epoch is an invariant: an edit never moves the clock.
 */
function applyEdit(nextSystem, { continuity = true } = {}) {
  const proposed = continuity
    ? withLambdaContinuity(loadedSystem, nextSystem, state.epochS)
    : structuredClone(nextSystem);
  const next = preflightEditedSystem(proposed); // throws by name; live world untouched
  const plan = classifyEdit(loadedSystem, next);
  const summary = {
    scope: plan.scope, systemKeys: plan.systemKeys, membership: plan.membership,
    bodies: Object.fromEntries([...plan.bodies].map(([id, v]) => [id, { keys: v.keys, classes: [...v.classes] }])),
    env: [...plan.env],
  };
  if (plan.scope === 'none') return { ...summary, identity: systemIdentity(loadedSystem) };

  if (plan.scope === 'system') {
    // Structural edits (membership, parent, star, nodes, resonances) take the
    // full validated setSystem teardown. setSystem re-derives the epoch for
    // the boot path — an EDIT must not move the clock, so restore it.
    const keep = { epochS: state.epochS, speed: state.speed, warp: state.warp };
    const identity = setSystem(next);
    state.epochS = keep.epochS; state.speed = keep.speed; state.warp = keep.warp;
    warpBands.reset(state.warp);
    return { ...summary, identity };
  }

  // --- per-body dispatch (scope 'bodies') ---
  const prevById = new Map(loadedSystem.bodies.map((b) => [b.id, b]));
  generationFence.bump();
  loadedSystem = next;
  let rebuildCurrent = false;
  for (const [id, info] of plan.bodies) {
    const prev = prevById.get(id);
    const body = loadedBodyById(id);
    const c = info.classes;
    const insolLevel = c.has('mechanics')
      ? Math.min(insolationMinLevel(prev), insolationMinLevel(body)) : Infinity;
    const bake = c.has('rebuild') ? 'full'
      : c.has('processes') ? 'bands'
        : c.has('mechanics') ? 'mechanics' : 'none';
    worker.postMessage(generationFence.stamp({ type: 'body', body: structuredClone(body), bake }));
    // main-side mirror of the worker's cache invalidation
    const minLevel = bake === 'full' ? 0
      : bake === 'bands' ? invalidationLevel(prev.processes ?? [], body.processes ?? [])
        : insolLevel;
    if (minLevel < 1) coarseResidency.delete(id);
    if (bake === 'full') { assetPacks.delete(id); figOkCache.delete(id); }
    // the disc map bakes from level-2 tiles + palette: stale under look,
    // band, rebuild and insolation-bearing mechanics edits
    if (c.has('look') || bake === 'bands' || bake === 'full' || insolLevel < Infinity) loadedDiscs.delete(id);
    if (c.has('clouds') || (c.has('mechanics') && hasSeasonalDecks(body))) cloudState.rows.delete(id);
    if (c.has('look') || c.has('clouds')) { msCache.delete(id); shineCovCache.delete(id); }
    if (id === state.body.id && (bake !== 'none' || c.has('look') || c.has('clouds'))) rebuildCurrent = true;
  }
  for (const id of plan.env) cloudState.rows.delete(id);
  resetAsyncPipelines();
  state.body = loadedBodyById(state.body.id) ?? state.body;
  if (cam) cam.body = state.body;
  if (rebuildCurrent) {
    // rebuild the current body's render residency under the new recipe, but an
    // edit is not a teleport: the camera pose survives verbatim.
    const pose = { lat: cam.lat, lon: cam.lon, alt: cam.alt, yaw: cam.yaw, pitch: cam.pitch, roll: cam.roll, free: cam.free };
    switchBody(state.body.id);
    Object.assign(cam, pose);
  } else if (state.tiles) {
    // tile streaming continues under the new generation (in-flight bakes died)
    state.tiles.systemGeneration = generationFence.current();
    state.tiles.pending.clear();
  }
  dispatchDiscJob();
  rebuildBodyMenu(state.body.id);
  if (systemView) systemView.setSystem(loadedSystem);
...
function switchBody(id) {
  ...
  state.tiles = new PlanetTiles(body, worker, shared, { atmSteps: FAST ? 6 : 14,
    generation: generationFence.current(), assetPack: assetPacks.get(body.id) ?? null });
  for (const root of coarseResidency.get(body.id)?.values() ?? []) state.tiles.onBaked(root);
  ...
  // count itself is gated per-frame on the keyframe row being loaded.
  {
    ...
    const row = layerForBody(bodyLayers, body.id);
...
const initialBody = loadedBodyById(initialBodyId);
const cam = new OrbitalCamera(canvas, initialBody, (dir) => (state.tiles ? state.tiles.heightAt(dir) : 0));
switchBody(initialBody.id);

let systemView = null;
function prewarmBody(bodyId) {
  const body = loadedBodyById(bodyId); if (!body) return false;
  // Root pyramid and whole-disc/cloud state remain worker-side until arrival.
  // PlanetTiles' normal requests then hit the baker's warm ancestor cache.
  if ((body.rocks || body.formations) && !assetPacks.has(bodyId) && !assetPending.has(bodyId)) {
    assetPending.add(bodyId); state.prewarmPending = assetPending.size;
    worker.postMessage(generationFence.stamp({ type: 'assetpack', bodyId, priority: 'background' }));
  }
  for (let face = 0; face < 6; face++) worker.postMessage(generationFence.stamp({ bodyId, face, level: 0, x: 0, y: 0, gen: 0, priority: 'background' }));
  if (!loadedDiscs.has(bodyId)) { discJobs.push(bodyId, -2); dispatchDiscJob(); }
  // Prewarm the representation the POST-travel warp will select (travel itself
  // runs at the clamped travelWarp; arrival restores savedWarp): warming live
  // keyframes for an above-band arrival would warm the wrong realm.
  if (body.clouds) {
    const postWarp = state.savedWarp ?? state.warp;
    if (warpRepresentation(postWarp).clouds === 'mean') requestCloudMean(body, meanBlockOf(body, state.epochS));
    else requestClouds(body, cloudKeyOf(body, state.epochS).k);
  }
  return true;
}
function leaveSystemView(bodyId) {
  if (bodyId === 'star') return;
  switchBody(bodyId);
  state.viewClass = 'surface'; state.hostId = bodyId; state.travel = null;
  state.speed = state.savedWarp ?? state.speed; state.savedWarp = null;
  state.warp = state.speed;
  // Continuous arrival seeds from the outgoing exposure; it is not a teleport.
  state.expSnap = 0; state.expSeed = false; state.ready = false; state.stable = 0;
  host.dispatchEvent(new CustomEvent('viewclasschange', { detail: { viewClass: 'surface', hostId: bodyId } }));
}
function enterSystemView(hostId = 'star') {
  state.viewClass = 'system'; state.hostId = hostId; state.ready = false; state.stable = 0;
  systemView.hostId = hostId;
  host.dispatchEvent(new CustomEvent('viewclasschange', { detail: { viewClass: 'system', hostId } }));
  return { viewClass: state.viewClass, hostId };
}
function travelTo(bodyId, options = {}) {
  if (bodyId !== 'star' && !loadedBodyById(bodyId)) throw new Error(`travelTo: unknown body '${bodyId}'`);
  if (state.viewClass !== 'system') enterSystemView(state.body.id);
  state.savedWarp = state.speed;
  prewarmBody(bodyId);
  const plan = systemView.beginTravel(bodyId, state.epochS, state.speed, options.durationScale ?? 1);
  // travel × warp law: warp clamps to the declared travel ceiling for the
  // travel duration and restores after — declared AND integrated alike.
  state.speed = state.warp = plan.travelWarp;
  state.travel = { bodyId, phase: 'departure', durationS: plan.durationS };
  return plan;
}
function lookAtBody(bodyId) {
  const p = hostRelativePosition(loadedSystem, bodyId, systemView.hostId, state.epochS);
  systemView.pose.pivot = p; return p.slice();
}
function applySystemView(spec = {}) {
  enterSystemView(spec.host ?? 'star');
  if (spec.reset) Object.assign(systemView.pose, starterSystemPose(loadedSystem));
  if (spec.epochS != null) state.epochS = spec.epochS;
  if (spec.warp != null) { state.speed = spec.warp; state.warp = spec.warp; warpBands.reset(spec.warp); }
  if (spec.range != null) systemView.pose.range = spec.range;
  if (spec.yaw != null) systemView.pose.yaw = spec.yaw * Math.PI / 180;
  if (spec.pitch != null) systemView.pose.pitch = spec.pitch * Math.PI / 180;
  systemView.pose.pivot = [0, 0, 0];
  if (spec.target) lookAtBody(spec.target);
  state.ready = false; state.stable = 0;
  return currentShotSpec('system view control');
}
function descendToSurface(spec = {}, durationS = 3) {
  if (state.viewClass !== 'surface') throw new Error('descendToSurface requires a body host');
  const from = { lon: cam.lon, lat: cam.lat, alt: cam.alt, yaw: cam.yaw, pitch: cam.pitch };
  const rad = Math.PI / 180;
  const to = {
    lon: spec.lon == null ? from.lon : spec.lon * rad,
    lat: spec.lat == null ? from.lat : spec.lat * rad,
    alt: Math.max(spec.alt ?? 1.8, 1.8),
    yaw: spec.yaw == null ? from.yaw : spec.yaw * rad,
    pitch: spec.pitch == null ? from.pitch : spec.pitch * rad,
  };
  let dl = to.lon - from.lon;
  while (dl > Math.PI) dl -= Math.PI * 2; while (dl < -Math.PI) dl += Math.PI * 2;
  const saved = state.speed; state.speed = 0; state.ready = false; state.stable = 0;
  const start = performance.now(), ease = (x) => x * x * (3 - 2 * x);
  return new Promise((resolve) => {
    const step = (now) => {
      const u = Math.max(0, Math.min(1, (now - start) / (Math.max(durationS, 0.05) * 1000))), t = ease(u);
      cam.lon = from.lon + dl * t; cam.lat = from.lat + (to.lat - from.lat) * t;
      cam.alt = Math.exp(Math.log(Math.max(from.alt, 1.8)) * (1 - t) + Math.log(to.alt) * t);
      cam.yaw = from.yaw + (to.yaw - from.yaw) * t; cam.pitch = from.pitch + (to.pitch - from.pitch) * t;
      if (u < 1) host.requestAnimationFrame(step);
      else { state.speed = saved; resolve(currentShotSpec('continuous descent')); }
    };
    host.requestAnimationFrame(step);
  });
}
systemView = new SystemView(renderer, camera, canvas, loadedSystem, {
  isActive: () => state.viewClass === 'system',
  onPick: (id) => travelTo(id),
  onHover: (marker) => host.dispatchEvent(new CustomEvent('navhover', { detail: marker })),
  onMarkers: (markers) => { state.navMarkers = markers; host.dispatchEvent(new CustomEvent('navmarkers', { detail: markers })); },
  onHost: (hostId) => { state.hostId = hostId; if (state.travel) state.travel.phase = 'target'; },
  onTravel: (detail) => host.dispatchEvent(new CustomEvent('travelstate', { detail })),
  onArrive: (id) => {
    if (id === 'star') {
      systemView.hostId = 'star'; state.hostId = 'star'; state.travel = null;
      state.speed = state.savedWarp ?? state.speed; state.savedWarp = null;
      state.warp = state.speed;
    } else leaveSystemView(id);
  },
});
...
function bindUI() {
  ...
  on('tday', (el) => {
    const next = +el.value;
    state.epochS += (next - state.tday) * spinPeriodS(state.body, loadedSystem);
    state.tday = next;
  });
  on('season', (el) => {
    const next = +el.value;
    state.epochS += (next - state.season) * orbitalPeriodS(state.body, loadedSystem);
    state.season = next;
  });
  on('speed', (el) => { state.speed = state.warp = +el.value; });
...
function currentShotSpec(note = 'defect capture') {
  // Phase E: an EDITED system travels inline with every spec (bookmark, F8
  // capture, __capture) so any pose in any edited system reproduces headlessly
  // — the recipeHash-known shipped systems stay implicit (URL/provenance).
  const systemPayload = state.systemEdited ? { system: structuredClone(loadedSystem) } : {};
  if (state.viewClass === 'system') return {
    viewClass: 'system', host: systemView.hostId, epochS: state.epochS, warp: state.warp,
    range: systemView.pose.range, yaw: systemView.pose.yaw * 180 / Math.PI,
    pitch: systemView.pose.pitch * 180 / Math.PI, pivot: systemView.pose.pivot.slice(), note,
    ...systemPayload,
  };
  const s = {
    body: state.body.id,
  ...
  // the declared warp travels with the spec whenever it is engaged (Phase W)
  if (state.warp) s.warp = state.warp;
...
const dlFile = (href, name) => { const a = doc.createElement('a'); a.href = href; a.download = name; a.click(); };
...
function capturePhoto() {
  ...
  renderPass(camera, cs.pos, cs.quat, [0, 0, width, height]);
  renderer.setScissorTest(false);
  dlFile(canvas.toDataURL('image/png'), `photo-${new Date().toISOString().replace(/[:.]/g, '-')}.png`);
  renderer.setPixelRatio(prev); renderer.setSize(width, height); makeTargets();
...
function playBookmarks() {
  ...
  const step = () => {
    ...
    if (A.epochS !== undefined && B.epochS !== undefined) state.epochS = lerp(A.epochS, B.epochS, f);
    else if (A.tday !== undefined && B.tday !== undefined) {
      state.tday = lerp(A.tday, B.tday, f);
      state.epochS = epochFromViews(state.body, { season: lerp(A.season ?? 0.15, B.season ?? 0.15, f), tday: state.tday }, loadedSystem);
    }
...
function renderBookmarks() {
  ...
  box.innerHTML = '';
  list.forEach((sp, i) => {
    const row = doc.createElement('div'); row.className = 'bmark';
    const b = doc.createElement('button');
    b.textContent = `${sp.body} · ${sp.alt >= 1000 ? (sp.alt / 1000).toFixed(0) + 'km' : Math.round(sp.alt) + 'm'}`;
    b.title = JSON.stringify(sp); b.onclick = () => host.__shot(sp);
    const x = doc.createElement('button'); x.textContent = '×'; x.className = 'bx';
    x.onclick = () => {
...
let disposed = false, frameRAF = 0;
function frame() {
  if (disposed) return;
  frameRAF = host.requestAnimationFrame(frame);
  ...
  if (state.speed !== 0) {
    ...
    // Signed since Phase W: negative warp runs the same closed-form world back.
    state.epochS += dt * state.speed;
  }
  // Phase W: the representation set for the declared warp. The live path steps
  // the hysteresis machine; captures reset it in applyScene/applySystemView.
  const timePolicy = warpBands.step(state.warp);
  {
    const views = viewsAtEpoch(body, state.epochS, loadedSystem);
    state.tday = views.tday;
    state.season = views.season;
    const el = $('tday');
    if (el && !el.matches(':active')) el.value = state.tday.toFixed(4);
  }
  if (state.viewClass === 'system') {
    state.frameNo++;
    const navFrame = systemView.update(state.epochS, now);
    if (state.viewClass === 'system' && navFrame) systemView.render();
    state.stable = navFrame && !state.travel ? state.stable + 1 : 0;
    state.ready = state.stable > 5;
    if (state.interactiveMs == null && navFrame) {
      state.interactiveMs = performance.now() - state.loadStarted;
      dispatchDiscJob();
    }
    const hud = $('hud');
    if (hud && state.frameNo % 10 === 0) hud.textContent = state.travel
      ? `travelling to ${state.travel.bodyId} · ${state.travel.phase}`
      : `${loadedSystem.star.name ?? 'Star'} system · true scale · ${state.speed.toLocaleString()}×`;
    return;
  }
  const eph = ephemeris(body, state.epochS, loadedSystem);
  shared.uSunDir.value.set(...eph.sunDirBF);
  const sc = loadedSystem.star.color;
  ...
  const { resolved: others, points: pointBodies } = assignBodyRepresentations(eph.others);
  skyUniforms.uNumBodies.value = others.length;
  bodyPointTier.update(pointBodies.map((o) => {
    const a = o.body.discAlbedo;
    const mean = Math.max((a[0] + a[1] + a[2]) / 3, 1e-6);
    let flux = discIntegratedFlux(o.irradiance, a, o.angRadius);
    let color = [sc[0] * a[0] / mean, sc[1] * a[1] / mean, sc[2] * a[2] / mean];
    // coma emission rides the SAME hand-down (Phase B): reflected + emitted
    // flux sum; the color is the flux-weighted mix. rAU falls out of the
    // ephemeris irradiance (irr = irr1AU·(AU/r)²), no second distance law.
    if (o.body.coma) {
      const rAU = Math.sqrt(loadedSystem.star.irradianceAt1AU / o.irradiance);
      const emission = comaApparentFlux(o.body.coma, rAU, o.dist, AU);
      if (emission > 0) {
        const cc = o.body.coma.color, total = flux + emission;
        color = color.map((v, i) => (v * flux + sc[i] * cc[i] * emission) / total);
        flux = total;
      }
    }
    return { dirBF: o.dirBF, flux, color };
  }));
  ...
  for (let i = 0; i < others.length; i++) {
    ...
    const M = mulMM(transpose(bodyToInertial(o.body, state.epochS, loadedSystem)), m);
    ...
    skyUniforms.uBodyRowV.value[i] = layerForBody(bodyLayers, o.body.id);
    skyUniforms.uBodyDiscLoaded.value[i] = loadedDiscs.has(o.body.id) ? 1 : 0;
    skyUniforms.uBodyFlatAlb.value[i].fromArray(o.body.discAlbedo);
    ...
    if (cb && !state.cloudsOff) {
      // Phase W: above the clouds band the companion disc reads the analytic
      // mean raster with canonical zero drift/frac (the packing makes the frac
      // lerp an identity; drift-averaged rows are rotation-invariant anyway).
      const meanMode = timePolicy.clouds === 'mean';
      const bk = meanMode ? null : cloudKeyOf(o.body, state.epochS);
      if (meanMode) requestCloudMean(o.body, meanBlockOf(o.body, state.epochS));
      else requestClouds(o.body, bk.k);
      const cRow = cloudState.rows.get(o.body.id);
      const bi = layerForBody(bodyLayers, o.body.id);
      const rowLive = cRow && (meanMode ? cRow.kind === 'mean' : cRow.kind === 'live' && cRow.k === bk.k);
      const nD = rowLive ? Math.min(cb.decks.length, MAX_DECKS) : 0;
      skyUniforms.uBodyCloudN.value[i] = nD;
      for (let d = 0; d < nD; d++) {
        const deck = cb.decks[d];
        skyUniforms.uBodyCloudA.value[i * 2 + d].set(
          meanMode ? 0 : driftPhase(deck, state.epochS), 2 * (deck.sigmaK ?? 0.003) * deck.thickM,
          bi * MAX_DECKS + d, meanMode ? 0 : bk.frac,
        );
      }
    ...
    if (gnt) {
      ...
      skyUniforms.uGiantBandN.value[i] = gnt.bands.length;
      for (let k = 0; k < 8; k++) {
        const bd = gnt.bands[k];
        skyUniforms.uGiantBand.value[i * 8 + k].set(bd ? bd.s : 0, bd ? bd.c[0] : 0, bd ? bd.c[1] : 0, bd ? bd.c[2] : 0);
      }
      skyUniforms.uGiantLimbExp.value[i] = gnt.limbExp;
      skyUniforms.uGiantLimbK.value[i] = gnt.limbK;
      const tDays = state.epochS / 86400, frac = (x) => x - Math.floor(x);
      ...
      const spinRate = 360 * 86400 / spinPeriodS(o.body, loadedSystem); // deg/day (System III, already in nB)
      ...
      skyUniforms.uGiantStorm.value[i].set(gnt.storm.lon, gnt.storm.lat, gnt.storm.r, sDrift);
      skyUniforms.uGiantStormCol.value[i].set(...gnt.storm.c);
      const hDrift = frac(omega(gnt.hexagon.latOn) * tDays / 360) * 2 * Math.PI;
      skyUniforms.uGiantHex.value[i].set(gnt.hexagon.latOn, gnt.hexagon.amp, hDrift, 0);
      skyUniforms.uGiantHexCol.value[i].set(...gnt.hexagon.c);
    } else skyUniforms.uBodyGiant.value[i] = 0;
    const rng = o.body.rings;
    if (rng) {
      skyUniforms.uBodyRing.value[i] = 1;
      const Rb = o.body.R, D = o.dist;
      skyUniforms.uRingInner.value[i] = Rb * rng.inner / D;
      skyUniforms.uRingOuter.value[i] = Rb * rng.outer / D;
      skyUniforms.uRingRp.value[i] = Rb / D; // exact planet R/D ratio for the shadow/occlusion tests (post-impl ring-1)
      for (let k = 0; k < 4; k++) {
        const gp = rng.gaps && rng.gaps[k];
        if (gp) skyUniforms.uRingGap.value[i * 4 + k].set(Rb * gp.r / D, Rb * gp.w / D, gp.depth, 1);
        else skyUniforms.uRingGap.value[i * 4 + k].set(0, 0, 0, 0);
      }
      skyUniforms.uRingCol.value[i].set(...rng.col);
      skyUniforms.uRingTau.value[i] = rng.tau;
      skyUniforms.uRingFsG.value[i] = rng.fscatterG;
    } else skyUniforms.uBodyRing.value[i] = 0;
  }

  shared.uTimeS.value = state.epochS % 4096;
  ...
  {
    if (timePolicy.aurora === 'mean') {
      // Phase W above-band form: canonical zero drift, pulse pinned to its
      // closed-form mean (the sin terms average to exactly zero) — a constant
      // per band, never the value at band entry (capture reproducibility law).
      shared.uAuroraPhase.value.set(0, AURORA_MEAN_PULSE);
    } else {
      const drift = (state.epochS * 0.03) % 4096;
      const pulse = 0.55 + 0.28 * Math.sin(state.epochS * 3.1e-4) + 0.22 * Math.sin(state.epochS * 7.3e-4 + 1.7);
    ...
    // Phase W: off above its declared bucket rate — the per-frame gate is
    // authoritative over switchBody's recipe upload (a pure fn of the declared
    // warp, so a frozen capture at high warp renders the storm dark).
    const lt = state.body?.clouds?.lightning;
    shared.uLightRate.value = lt && timePolicy.lightningOn ? (lt.rate ?? 0.02) : 0;
    const lp = lt?.period ?? 3.0;
    const bucket = Math.floor(state.epochS / lp);
    const bseed = ((Math.imul(bucket, 2654435761) >>> 0) % 100000) / 1000; // [0,100)
    shared.uLightBucket.value.set(bseed, state.epochS / lp - bucket);
  }
  ...
  // round-2 "twilight dash" defect — per-pixel acos cancellation noise).
  {
    ...
    for (const o of others) {
      if (no >= MAX_ECLIPSE_OCCLUDERS) break;
  ...
  for (let oi = 0; oi < others.length; oi++) {
    ...
    if (cbS && cRowS && !state.cloudsOff) {
      const tq = Math.floor(state.epochS / 60);
      let cc = shineCovCache.get(o.body.id);
      if (!cc || cc.tq !== tq) {
        const Mt = mulMM(transpose(bodyToInertial(o.body, state.epochS, loadedSystem)), m);
        const mv = (v) => [
        ...
        for (let d = 0; d < Math.min(cbS.decks.length, MAX_DECKS) && d < cRowS.decks; d++) {
          ...
          const aD = alphaMeanLit(o.body, cRowS.rgba, d, state.epochS, toUs, toSun);
  ...
  // for the OWN body. All time inputs derive from state.epochS (frozen by __shot);
  ...
  {
    const cl = body.clouds;
    if (cl && !state.cloudsOff && timePolicy.clouds === 'mean') {
      // Phase W above-band representation: the worker's analytic time-mean
      // raster (equivalent coverage through the alpha law — Jensen-honest by
      // construction). Canonical zero lerp/drift; the last delivered block
      // keeps rendering while the next generates (the mean-row analog of the
      // k−1 rollover fallback below); the kind tag keeps a mean raster from
      // ever rendering as a keyframe pair after a downward crossing (F2 class).
      requestCloudMean(body, meanBlockOf(body, state.epochS));
      const rowC = cloudState.rows.get(body.id);
      shared.uCloudLerp.value = 0;
      shared.uCloudDecks.value = rowC && rowC.kind === 'mean' ? Math.min(cl.decks.length, MAX_DECKS) : 0;
      for (let d = 0; d < MAX_DECKS; d++) shared.uCloudDeckB.value[d].x = 0;
    } else if (cl && !state.cloudsOff) {
      const { k, frac } = cloudKeyOf(body, state.epochS);
      ...
      const live = rowC && rowC.kind === 'live' && (rowC.k === k || rowC.k === k - 1);
      shared.uCloudLerp.value = rowC && rowC.kind === 'live' && rowC.k === k - 1 ? 1 : frac;
      shared.uCloudDecks.value = live ? Math.min(cl.decks.length, MAX_DECKS) : 0;
      for (let d = 0; d < MAX_DECKS; d++) {
        shared.uCloudDeckB.value[d].x = cl.decks[d] ? driftPhase(cl.decks[d], state.epochS) : 0;
      }
    } else {
  ...
  const mainPixAng = (camera.fov * Math.PI / 180) / (viewportSize().height * renderer.getPixelRatio());
  ...
  // Phase W: ocean wave time pins to its canonical closed-form value above the
  // band (a statistically identical sea, one fixed phase draw); below it, the
  // live epoch passes through untouched — byte-identical at warp 0.
  const stats = state.tiles.update(cams, oceanTimeS(state.epochS, timePolicy), ++state.frameNo, mainPixAng, turbo,
    Math.min(state.lastMs / 1000, 2));
  if (state.interactiveMs == null && stats.baked > 0) {
    state.interactiveMs = performance.now() - state.loadStarted;
    dispatchDiscJob();
  }
  ...
  if (meterNow) {
    ...
    if (mtr && stats.baked > 0) {
      ...
      if (Math.abs(Math.log(r)) > 0.02) {
        ...
        // (blown frames are unusable), brightening is cautious. Phase W: above
        // the declared AE band the time constant stretches (× aeGain) so the
        // servo converges to the time-averaged scene instead of chasing a
        // sun that sweeps the sky in seconds; teleport snaps stay instant.
        const gain = state.expSnap > 0 ? 0.9 : (r < 1 ? 0.5 : 0.18) * timePolicy.aeGain;
  ...
  const settled = stats.pending === 0 && state.tiles.rockQueue.length === 0
    && state.tiles.formQueue.length === 0
    && !stats.fading && others.every((o) => loadedDiscs.has(o.body.id)) && cloudState.pending === 0;
...
host.__state = state; host.__shared = shared; // debug/inspection access
host.__ready = () => state.ready;
host.__interactive = () => state.interactiveMs != null;
// per-subsystem frame timing (EMA ms) for the motion bench's perf-budget gate
host.__perf = () => ({ ...state.perf });
...
host.__stream = () => {
  ...
  return {
    ...
    prewarmQ: state.prewarmPending,
    coarseBodies: coarseResidency.size,
    workerTiles: state.workerCache.tiles, workerBudget: state.workerCache.budget,
    workerBakers: state.workerCache.bakers, workerEvictions: state.workerCache.evictions,
    systemGeneration: generationFence.current(), skyUniformHeadroom: skyUniformBudget.headroom,
    interactiveMs: state.interactiveMs,
    warp: state.warp, cloudKind: cloudState.rows.get(state.body.id)?.kind ?? null,
    warpBands: warpBands.state(), lightRate: shared.uLightRate.value,
    systemEdited: state.systemEdited,
  };
};
...
host.__cloud = (dir, sunDir) => {
  ...
  return {
    k: row.k, kind: row.kind,
    cov: cloudCovJS(b, row.rgba, 0, dir, state.epochS),
    shade: cloudShadeJS(b, row.rgba, gp, sunDir ?? [shared.uSunDir.value.x, shared.uSunDir.value.y, shared.uSunDir.value.z], state.epochS),
  };
};
...
host.__recipe = () => JSON.parse(JSON.stringify(state.body.processes));
host.__reload = (processes) => {
  ...
  state.discPending++;
  loadedDiscs.delete(state.body.id);
  worker.postMessage(generationFence.stamp({ type: 'discmap', bodyId: state.body.id }));
  state.ready = false; state.stable = 0;
  return true;
};
...
function faceSunYaw() {
  const s = ephemeris(state.body, state.epochS, loadedSystem).sunDirBF;
...
function applyScene(spec = {}) {
 return new Promise((resolve) => {
  const applied = resolveSpec(spec, { tday: state.tday });
  // Phase E reproducibility: spec.system selects the world before anything is
  // posed in it — an id string resolves through the canonical registry, an
  // inline payload (an EDITED system) applies via the full setSystem preflight.
  // No-op when the named/loaded hashes already match.
  if (applied.system != null) {
    const want = typeof applied.system === 'string'
      ? (applied.system === loadedSystem.id ? null : systemsById.get(applied.system))
      : applied.system;
    if (typeof applied.system === 'string' && applied.system !== loadedSystem.id && !want) {
      throw new Error(`applyScene: unknown system id '${applied.system}' — pass an inline spec.system payload`);
    }
    if (want && systemIdentity(want).recipeHash !== systemIdentity(loadedSystem).recipeHash) setSystem(want);
  }
  if (applied.body && applied.body !== state.body.id) switchBody(applied.body);
  state.tday = applied.tday ?? viewsAtEpoch(state.body, state.epochS, loadedSystem).tday;
  ...
  state.season = applied.season;
  const seasEl = $('season'); if (seasEl) seasEl.value = String(state.season);
  // Phase W capture law: the clock freezes (speed 0, frozen-epoch stepping)
  // while the DECLARED warp selects the representation set — a spec at warp
  // 1e6 renders the above-band forms and the gate can see them. Selection
  // resets to the pure function of the declared value (no live-path history).
  state.speed = 0;
  state.warp = applied.warp;
  warpBands.reset(state.warp);
  ...
  cam.set({ ...applied, alt: applied.alt ?? state.body.camera.alt });
  // photo-mode [camera] controls reset like every other field
  applyWB(applied.wb);
  state.meterMode = applied.meter;
  state.wbMode = applied.wbMode;
  shared.uGrade.value = state.grade = applied.grade;
  if (applied.phaseDeg !== undefined) state.tday = solvePhaseTday(state.body, applied.phaseDeg, cam.surfaceDir(), (b, t) => ephemeris(b, t, loadedSystem), state.season);
  state.epochS = applied.epochS != null
    ? applied.epochS
    : epochFromViews(state.body, { season: state.season, tday: state.tday }, loadedSystem);
  if (applied.faceSun) cam.yaw = faceSunYaw() + (applied.yaw * Math.PI) / 180;
  camera.fov = applied.fov; camera.updateProjectionMatrix();
  state.lookAtBody = applied.lookAt;
  state.fixedEV = applied.fixedEV;
  setClean(applied.clean);
  const mode = applied.mode;
  setMode(typeof mode === 'string' ? MODES.indexOf(mode) : mode);
  { const v = applied.wire; state.tiles.wireframe = v; const el = $('wire'); if (el) el.checked = v; }
  { const v = applied.debris; state.tiles.debris = v; const el = $('debris'); if (el) el.checked = v; }
  ...
  state.cloudsOff = !applied.clouds;
  { const v = applied.inset; setInset(v); const el = $('inset'); if (el) el.checked = v; }
  state.exposureBias = applied.exposure;
  ...
  const wait = () => {
    ...
    if (settled || performance.now() - t0 > (applied.waitMs ?? 150000)) {
...
host.__shot = applyScene;
host.__capture = () => currentShotSpec('capture');
host.__system = () => systemIdentity(loadedSystem);
host.__setSystem = (system) => setSystem(system);
// Phase E editor hooks: the panel (and the harness) read a CLONE, mutate it,
// and hand a whole next-system back — nothing ever reaches the live recipe
// except through applyEdit's preflight/classify/dispatch path.
host.__editorSystem = () => structuredClone(loadedSystem);
host.__editSystem = (next, opts) => applyEdit(next, opts);
host.__editPlan = (next) => {
  const plan = classifyEdit(loadedSystem, preflightEditedSystem(next));
  return { scope: plan.scope, systemKeys: plan.systemKeys, membership: plan.membership,
    bodies: Object.fromEntries([...plan.bodies].map(([id, v]) => [id, { keys: v.keys, classes: [...v.classes] }])),
    env: [...plan.env] };
};
// edit-isolation witness: FNV over a body's baked disc-atlas row (the disc is
// itself baked from the level-2 tiles + palette, so this hashes both).
host.__discHash = (bodyId) => {
  const row = bodyLayers.get(bodyId);
  if (row == null || row >= NB || !loadedDiscs.has(bodyId)) return null;
  const base = row * DISC_W * DISC_H * 4;
  let h = 0x811c9dc5;
  for (let i = base, end = base + DISC_W * DISC_H * 4; i < end; i++) {
    h ^= discAtlas[i]; h = Math.imul(h, 0x01000193);
  }
  return (h >>> 0).toString(16).padStart(8, '0');
};
host.__nav = () => ({ viewClass: state.viewClass, hostId: state.hostId,
  markers: state.navMarkers.map((m) => ({ ...m })), travel: state.travel ? { ...state.travel } : null,
  pose: { ...systemView.pose, pivot: systemView.pose.pivot.slice() } });
host.__enterSystemView = (hostId = 'star') => enterSystemView(hostId);
host.__travelTo = (bodyId, options) => travelTo(bodyId, options);
host.__lookAtBody = (bodyId) => lookAtBody(bodyId);
host.__systemShot = (spec) => applySystemView(spec);
host.__descendToSurface = (spec, durationS) => descendToSurface(spec, durationS);
host.__input = async (steps = []) => {
  state.speed = 0;
  const ordered = [...steps].sort((a, b) => (a.frame ?? 0) - (b.frame ?? 0));
  const last = ordered.length ? ordered[ordered.length - 1].frame ?? 0 : 0;
  let at = 0;
  for (let frameNo = 0; frameNo <= last; frameNo++) {
    while (at < ordered.length && (ordered[at].frame ?? 0) === frameNo) {
      const s = ordered[at++];
      const target = s.target === 'window' ? host : canvas;
      const init = { bubbles: true, cancelable: true, ...s };
      const event = s.type.startsWith('key')
        ? new host.KeyboardEvent(s.type, init)
        : s.type === 'wheel'
          ? new host.WheelEvent(s.type, init)
          : new host.PointerEvent(s.type, init);
      target.dispatchEvent(event);
    }
    await new Promise((resolve) => host.requestAnimationFrame(resolve));
  }
  return currentShotSpec('scripted-input');
};
...
return {
  ...
  applySystemView,
  descendToSurface,
  capture: currentShotSpec,
  resize: onResize,
  destroy() {
    disposed = true;
    host.cancelAnimationFrame(frameRAF);
    host.removeEventListener('resize', onResize);
    if (state.tiles) state.tiles.dispose();
    systemView.destroy();
    bodyPointTier.dispose();
    discTex.dispose(); cloudTex.dispose();
    worker.terminate();
    renderer.dispose();
  },
};
